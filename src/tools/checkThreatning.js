import choiceAfterCheck from "@/movement/choiceAfterCheck";
...
function checkHowThreatOnEachSquare(board) {
    ...
    if (kingsSquareToDefend) {
        choiceAfterCheck.choicesAfterCheck(kingsSquareToDefend,board)
        kingsSquareToDefend = {}
    }
}
...
function checkKnightNextMoveSquare(square, board, i, j) {
    if (square.x + j > 7 || square.x + j < 0 || square.y + i > 7 || square.y + i < 0) return
    let sideSquare = board[square.x + j][square.y + i]
    declareOnThreaten(square, sideSquare, board)
}

function declareOnThreaten(threateningSquare, threatenedSquare, board) {
    let threatenPiece = {
    ...
    if (threatenedSquare.piece && threatenedSquare.piece.letterName === 'k' && threatenedSquare.piece.color !== threateningSquare.piece.color) {
        console.log('check!')
        kingsSquareToDefend = threatenedSquare
    }
}